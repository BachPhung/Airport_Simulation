...
Airport_2::Airport_2(int end_time, int queue_limit, double arrival_rate, double departure_rate) {
	arrival_runway = new Runway(queue_limit);
	departure_runway = new Runway(queue_limit);
}
...
void Airport_2::shut_down() {
    cout << "-----------ARRIVAL RUNWAY-----------" << endl;
    this->arrival_runway->shut_down(end_time);
    cout << "------------------------------------" << endl;
    cout << "-----------DEPARTURE RUNWAY-----------" << endl;
    this->departure_runway->shut_down(end_time);
}
