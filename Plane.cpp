...
Plane::Plane(int flt, int time1, Plane_status status, int fuel) {
	...
	if (status == arriving)
		...
		cout << "take off ";
	cout << "(fuel:" << this->fuel << ") remaining" << endl;
}
Plane::Plane()
/*
Post:  The Plane data members flt_num, clock_start,
...
void Plane::setTimeMoved(int time) {
	this->time_moved = time;
}
int Plane::getTimeMoved() {
	return this->time_moved;
}
